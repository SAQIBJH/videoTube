...
function AllVideo() {
  ...
  if (isError) {
    return <div className="flex justify-center items-center w-full h-screen">Error: {error.message}</div>;
  }

  return (
    <section className="container border mt-5">
      <div className="grid gap-8  max-md:max-w-full" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))' }}>
        {videos.length > 0 ? (
          videos.map((video) => (
